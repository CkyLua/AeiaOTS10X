			...
			if isInArray(annihilatorReward, item.uid) == TRUE then
				if getPlayerStorageValue(cid, 30015) == -1 then
					if playerCap >= itemWeight then
						...
						doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, 'You found a ' .. getItemName(item.uid) .. '.')
						setPlayerStorageValue(cid, 30015, 1)
					else
						doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, 'You found a ' .. getItemName(item.uid) .. ' weighing ' .. itemWeight .. ' oz it\'s too heavy.')
					end
			else
