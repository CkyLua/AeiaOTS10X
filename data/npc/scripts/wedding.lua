    ...
    elseif playerStatus == PROPOSED_STATUS then
        ...
        elseif candidate == player:getGuid() then
            ...
            elseif candidateStatus == PROPOSED_STATUS then
                ...
                --player:sendTextMessage(MESSAGE_INFO_DESCR, ">>> " .. candidate)
                npcHandler:say('Ok, now let\'s wait and see if {' ..  getPlayerNameById(candidate) .. '} accepts your proposal.', cid)
...
local function confirmRemoveEngage(cid, message, keywords, parameters, node)
    ...
    if playerStatus == PROPOSED_STATUS then
        ...
        node:addChildKeyword({'yes'}, removeEngage, {moveup = 3, text = 'Ok, your marriage proposal to {' .. getPlayerNameById(playerSpouse) .. '} has been removed.'})
    else
...
local node1 = keywordHandler:addKeyword({'marry'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Would you like to get married?'})
