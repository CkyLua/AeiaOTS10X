...
function onUse(cid, item, fromPosition, itemEx, toPosition)
    ...
    if(item.itemid == 7760 and isInArray({2342}, itemEx.itemid)) then
        doTransformItem(itemEx.uid, 2343)
        doRemoveItem(item.uid, 1)
    ...
    if(item.itemid == 7762 and isInArray({6530, 10021}, itemEx.itemid)) then
        doTransformItem(itemEx.uid, 6132)
        doRemoveItem(item.uid, 1)
    ...
    if(isInArray(enchantableGems, item.itemid)) then
        ...
        if(not getPlayerFlagValue(cid, PlayerFlag_NotGainMana) and (not getTileInfo(getThingPos(cid)).hardcore or config.hardcoreManaSpent)) then
