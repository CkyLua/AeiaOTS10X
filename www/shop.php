...
if (!empty($_POST['buy'])) {
	if ($user_znote_data['points'] >= $buy['points']) {
		?><td>You have <?php echo (int)($user_znote_data['points'] - $buy['points']); ?> points. (<a href="buypoints.php">Donate</a>).</td><?php
	} else {
		?><td>You have <?php echo $user_znote_data['points']; ?> points. (<a href="buypoints.php">Donate</a>).</td><?php
	}
} else {
	?><td>You have <?php echo $user_znote_data['points']; ?> points. (<a href="buypoints.php">Donate</a>).</td><?php
}
...
<table>
	<tr class="yellow">
		...
		<form action="" method="POST">
			...
			<input type="submit" value="  DONATE  "  class="needconfirmation" data-item-name="<?php echo $offers['describtion']; ?>" data-item-cost="<?php echo $offers['points']; ?>">
		</form>
		<?php
		echo '</td>';
		echo '</tr>';
...
<?php }
} else echo '<h1>Donation system disabled.</h1><p>Sorry, this functionality is disabled.</p>';
