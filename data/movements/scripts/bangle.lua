...
function coinsLeftCheck(cid)
    ...
    if(isPlayer(cid) and getPlayerSlotItem(cid, CONST_SLOT_RING).itemid == config.itemId) then				
		doPlayerRemoveItem(cid, config.coinItemId, config.removeCoinsPerInterval)
		doSendMagicEffect(pos, CONST_ME_YELLOW_RINGS)
		doSendMagicEffect(pos, CONST_ME_BLOCKHIT)
		if(getPlayerItemCount(cid, config.coinItemId) < config.removeCoinsPerInterval) then
...
function initRing(cid)
	local pos = getCreaturePosition(cid)
    if(isPlayer(cid) and getPlayerStorageValue(cid, config.storage) == 1 and getPlayerSlotItem(cid, CONST_SLOT_RING).itemid == config.itemId) then
		doSendDistanceShoot(pos, {x = pos.x, y = pos.y - 3, z = pos.z, stackpos = 255}, CONST_ANI_SMALLHOLY)
		doSendDistanceShoot(pos, {x = pos.x, y = pos.y + 3, z = pos.z, stackpos = 255}, CONST_ANI_SMALLHOLY)
		doSendDistanceShoot(pos, {x = pos.x - 3, y = pos.y, z = pos.z, stackpos = 255}, CONST_ANI_SMALLHOLY)
		doSendDistanceShoot(pos, {x = pos.x + 3, y = pos.y, z = pos.z, stackpos = 255}, CONST_ANI_SMALLHOLY)
		...
		if(getPlayerSlotItem(cid, CONST_SLOT_RING).itemid == config.itemId) then
